...
if [[ $CFG =~ ^[Yy]$ ]]; then
    ...
    # check for existing config folders and backup
    for DIR in cava foot hypr mako swappy swaylock waybar wlogout wofi; do
        DIRPATH=~/.config/$DIR
        if [ -d "$DIRPATH" ]; then
    ...
    cp -r config/cava ~/.config/ || {
        echo "Error: Failed to copy cava config files."
        exit 1
    } 2>&1 | tee -a "$LOG"
    cp -r config/swappy ~/.config/ || {
        echo "Error: Failed to copy swappy config files."
        exit 1
    } 2>&1 | tee -a "$LOG"
