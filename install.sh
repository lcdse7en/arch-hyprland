#!/usr/bin/env bash

#********************************************
# Author      : lcdse7en                    *
# E-mail      : 2353442022@qq.com           *
# Create_Time : 2023-12-10 19:05:15         *
# Description : Install hyprland script     *
#********************************************

yay -S hyprland-git polkit-kde-agent dunst grimblast rofi rofi-emoji wl-clipboard wf-recorder wlogout grimblast-git hyprpicker-git hyprpaper-git xdg-desktop-portal-hyprland-git ffmpegthumbnailer tumbler wtype colord imagemagick swaylock-effects qt5-wayland qt6-wayland ripgrep waybar-hyprland-git cliphist
# themes
yay -S catppuccin-gtk-theme-mocha catppuccin-cursors-mocha catppuccin-mocha-grub-theme-git nwg-look

# apps
yay -S cava pavucontrol ranger zsh starship neovim viewnior noise-suppression-for-voice cliphist

yay -S thunar thunar-archive-plugin file-roller gvfs thunar-volman

# Hyprland Stuff
yay -S hyprland-git hyprpicker-git waybar-git dunst nwg-look wf-recorder wlogout wlsunset

# Dependencies
yay -S colord ffmpegthumbnailer gnome-keyring grimblast-git gtk-engine-murrine \
    imagemagick kvantum pamixer playerctl polkit-kde-agent qt5-quickcontrols \
    qt5-quickcontrols2 qt5-wayland qt6-wayland swaybg ttf-font-awesome tumbler \
    ttf-jetbrains-mono ttf-icomoon-feather xdg-desktop-portal-hyprland-git xdotool \
    xwaylandvideobridge-cursor-mode-2-git cliphist qt5-imageformats qt5ct

# CLI & Tools
yay -S btop cava neofetch noise-suppression-for-voice \
    rofi-lbonn-wayland-git rofi-emoji starship zsh viewnior ocs-url

## Browser & File Explorer
paru -S brave-bin file-roller noto-fonts noto-fonts-cjk \
    noto-fonts-emoji thunar thunar-archive-plugin

# VSCode
paru -S code code-features code-marketplace

# Theme Based
paru -S colloid-gtk-theme-git papirus-icon-theme sddm-git swaylock-effects-git
